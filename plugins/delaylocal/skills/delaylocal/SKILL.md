...
> 本 skill 內的 js 路徑用 `${CLAUDE_PLUGIN_ROOT}/skills/delaylocal/`。`CLAUDE_PLUGIN_ROOT` 是
> Claude Code 安裝 plugin 時注入的環境變數，指向本 plugin 根目錄。

## 前置設定（一次性，每台機器）

LINE 憑證**不進 git**，要在本機自行設定其一：

- **設定檔（建議）**：複製 `${CLAUDE_PLUGIN_ROOT}/skills/delaylocal/notify-line.config.example.json`
...
### 3. 跑主工具拿 JSON
```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/delaylocal/delaylocal.js" [bufferSeconds] --prompt-file <步驟2的唯一檔名>
```
