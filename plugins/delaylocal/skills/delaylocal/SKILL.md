...
檔名規則：`delaylocal-input-<14位時間戳>-<4位隨機>.txt`，放在**系統暫存目錄**（不要寫死使用者名；Windows 為 `%TEMP%`，實際寫檔時用該機器展開後的絕對路徑，通常是 `C:\Users\<你的帳號>\AppData\Local\Temp\`）。
例如：`%TEMP%\delaylocal-input-20260520213045-7f3a.txt`
...
- **第一行** = `/goal <你的完成條件>；並且已執行 notify-line.js 完成收尾通知（有設 LINE 憑證→發出總結並回應 200；未設→工具自動略過、視同此步完成）`
  —— 把「已收尾通知」**納入完成條件**：goal 達成後會自動清除、不接後續指示，所以唯有把這步寫進條件，
  goal 引擎才會強迫自己做完才肯停（未設憑證時 notify-line.js 回 exit 0，同樣算達成、不會卡住）。
